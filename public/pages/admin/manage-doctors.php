...
if(strlen($_SESSION['id'] == 0)) {
    ...
    // API handling
    if (isset($_GET['api']) && $_GET['api'] == 'doctors') {
        ...
        switch ($method) {
            ...
            case 'POST':
                ...
                if (isset($data['specilization']) && isset($data['doctorName'])) {
                    $specilization = mysqli_real_escape_string($con, $data['specilization']);
                    $doctorName = mysqli_real_escape_string($con, $data['doctorName']);
                    $creationDate = date('Y-m-d H:i:s');

                    $query = "INSERT INTO doctors (specilization, doctorName, creationDate) VALUES ('$specilization', '$doctorName', '$creationDate')";
            ...
            case 'PUT':
                ...
                $data = json_decode(file_get_contents('php://input'), true);
                if (isset($data['id']) && isset($data['specilization']) && isset($data['doctorName'])) {
                    $id = $data['id'];
                    $specilization = mysqli_real_escape_string($con, $data['specilization']);
                    $doctorName = mysqli_real_escape_string($con, $data['doctorName']);
                    
                    $query = "UPDATE doctors SET specilization = '$specilization', doctorName = '$doctorName' WHERE id = '$id'";
                    mysqli_query($con, $query);
        ...
        exit; // Exit after processing the API request
    }
